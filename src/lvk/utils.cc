...
std::vector<VkLayerProperties> lvk::get_vulkan_layer_support() {
	...
	if (layer_cnt) {
		...
		lvk_throw_if_failed(vkEnumerateInstanceLayerProperties(&layer_cnt, layer_list.data()));
	}
	return layer_list;
}
...
bool lvk::is_all_layers_supported(const std::vector<str> &layers) {
	std::set<str> set;
	for (const auto layer_list = get_vulkan_layer_support(); const auto &prop : layer_list)
...
std::string lvk::vk_result_to_string(VkResult result) {
	...
	default:
		return "LVK_UNKNOWN_VK_RESULT";
	}
}

std::vector<VkPhysicalDevice> lvk::get_vulkan_physical_devices(VkInstance const &instance_handle) {
	uint32_t count;
