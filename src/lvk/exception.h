...
#define lvk_throw_if_failed_ex(action, message) {\
	...
	if (__result != VK_SUCCESS) {\
		throw lvk::vk_auto_exception(message, __result, #action, __func__, __FILE__, __LINE__);\
	}\
} void(0)

#define lvk_throw_if_failed(action) lvk_throw_if_failed_ex(action, lvk::vk_auto_exception::no_message)
