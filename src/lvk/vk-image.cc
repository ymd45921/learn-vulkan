...
namespace lvk {
	...
	VkDeviceMemory vk_image2d::allocate_memory(const VkMemoryPropertyFlags &properties) {
		...
		allocate_info.memoryTypeIndex =
			base->physical_device().pick_memory_type(requirements.memoryTypeBits, properties);
		vkAllocateMemory(**base, &allocate_info, default_vk_allocation_callbacks, &memory);
		return memory;
	}
	...
	vk_image2d vk_image2d::create(const VkImageCreateInfo &create_info, device *device,
		...
		VkImage handle;
		lvk_throw_if_failed_ex(vkCreateImage(**device, &create_info, default_vk_allocation_callbacks, &handle),
